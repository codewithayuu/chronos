...
FILTERABLE_VITALS = ["heart_rate", "spo2",
...
class DrugFilter:
    ...
    def apply(
        ...
        for drug in state.active_drugs:
            ...
            for vital_name in affected_vitals:
                ...
                expected_change = self.drug_db.get_expected_change(
                    drug.drug_name, vital_name)
                ...
                if self._is_masking(vital_detail, actual_change, expected_change):
                    ...
                    if vital_name not in masked_vitals:
                        ...
                        drug_annotations.append(
                            f"⚠ {drug.drug_name} may be masking decline in {
                    ...
                    if vital_name not in explained_vitals:
                        ...
                        drug_annotations.append(
                            f"{vital_name.replace('_', ' ')} change explained by {
                                drug.drug_name}"
                        )
    ...
    def _is_within_effect_window(self, drug: DrugEffect, current_time: datetime) -> bool:
        ...
        onset_time = drug.start_time + \
            timedelta(minutes=drug_entry.onset_minutes)
        end_time = drug.start_time + \
            timedelta(minutes=drug_entry.onset_minutes +
                      drug_entry.duration_minutes)
    ...
    def _is_change_explained(self, actual_change: float, expected_change: float) -> bool:
        ...
        # Within tolerance?
        tolerance = abs(expected_change) * \
            (1.0 + self.config.tolerance_fraction)
    ...
    def _build_message(
        ...
        if abs(adjusted_ces - raw_ces) > 0.01:
            return (
                f"{base_msg} (CES adjusted from {
                    raw_ces:.2f} to {adjusted_ces:.2f} "
