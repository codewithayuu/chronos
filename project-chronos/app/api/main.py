...
def create_app() -> FastAPI:
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        try:
            ...
            # Send historical sparklines so charts populate instantly
            initial_sparklines = {}
            for pid in manager.latest_states:
                history = manager.get_patient_history(pid, hours=2)
                ces_list = [round(s.composite_entropy, 3) for s in history if not s.calibrating][-120:]
                if ces_list:
                    initial_sparklines[pid] = ces_list
            
            if initial_sparklines:
                await websocket.send_json(jsonable_encoder({
                    "event": "initial_sparklines",
                    "data": initial_sparklines,
                }))
